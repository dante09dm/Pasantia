        ...
        <v-form v-model="valid">
            <v-row>
                <v-avatar class="ma-5" size="130">
                    <v-img v-if="user.data.afiliado[0].foto"
                        :src="`data:image/png;base64,${user.data.afiliado[0].foto}`" />
                    <v-img v-else src="@/assets/blank-user-profile.png" />
                </v-avatar>
            ...
            <v-row>
                <v-col cols="12" md="4">
                    <v-text-field v-model="user.first_name" :rules="nameRules" :counter="35" label="Nombres" required
                        :disabled="!enable">
                    </v-text-field>
                </v-col>
                <v-col cols="12" md="5">
                    <v-text-field v-model="user.last_name" :rules="lastnameRules" :counter="35" label="Apellido"
                        required :disabled="!enable">
                    </v-text-field>
                </v-col>
                <v-col cols="3" md="1">
                    <v-select v-model="user.type_dni" :items="items_tipo" label="Tipo Doc." :disabled="!enable">
                    </v-select>
                </v-col>
                <v-col cols="9" md="2">
                    <v-text-field v-model="user.id_number" ref="dniNumber" type="number" label="Nro." required
                        :disabled="!enable">
                    </v-text-field>
                </v-col>
            </v-row>
            <v-row>
                <v-col cols="12" md="2">
                    <v-text-field v-model="user.id_number" ref="dniNumber" type="number" label="CUIL" required
                        :disabled="!enable">
                    </v-text-field>
                ...
                <v-col cols="5" md="2">
                    <DatePicker v-if="estado_civil !== 'Soltero/a/x'" text="Fecha de Inicio" />
                </v-col>
            </v-row>
            <v-row>
                <v-spacer></v-spacer>
